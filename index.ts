const memeEl = document.getElementById("meme-el") as HTMLElement;
const jokeEl = document.getElementById("joke-el") as HTMLElement;
const quoteEl = document.getElementById("quote-el") as HTMLElement;
const riddleEl = document.getElementById("riddle-el") as HTMLElement;
const riddleReveal = document.getElementById("riddle-reveal") as HTMLElement;
...
function showMeme(): void {
  ...
  memeImg.id = "meme-img";
  filter("meme-img");
  memeImg.src = randomMemeUrl.toString();
  memeEl.insertAdjacentElement("afterend", memeImg);
}
...
function showJoke(): void {
  ...
  jokePara.id = "jokes";
  filter("jokes");
  jokePara.textContent = randomJoke.toString();
  jokeEl.insertAdjacentElement("afterend", jokePara);
}

function showQuote(): void {
  clearAll();
  const randomQuote = getRandomData("quotes") as {
    quote: string;
    author: string;
  };
  const quotePara = document.createElement("p");
  quotePara.id = "random-quote";
  filter("random-quote");
  quotePara.textContent = randomQuote.quote;
  quoteEl.insertAdjacentElement("afterend", quotePara);

  const author = document.createElement("p");
  author.id = "quote-author";
  filter("quote-author");
  author.textContent = `- ${randomQuote.author}`;
  quotePara.insertAdjacentElement("afterend", author);
}

function showRiddle(): void {
  clearAll();
  const randomRiddle = getRandomData("riddles") as {
    question: string;
    answer: string;
  };
  const riddlePara = document.createElement("p");
  riddlePara.id = "riddle-para";
  filter("riddle-para");
  filter("riddleAnswer");
  riddlePara.textContent = randomRiddle.question;
  riddleEl.insertAdjacentElement("afterend", riddlePara);

  const riddleQue = document.getElementById("riddle-para");
  ...
  riddleQue?.insertAdjacentElement("afterend", riddleAnswer);
  riddleAnswer.hidden = true;
  riddleReveal.style.cursor = "pointer";

  riddleReveal.addEventListener("click", () => {
    riddleAnswer.hidden = false;
    riddleReveal.style.cursor = "not-allowed";
  });
}

type DataType = "memes" | "jokes" | "quotes" | "riddles";

function getRandomData(type: DataType) {
  return data[type][rn(data[type].length)];
}

function rn(len: number) {
